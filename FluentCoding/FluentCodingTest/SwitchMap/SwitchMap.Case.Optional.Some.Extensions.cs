...
namespace FluentCodingTest.SwitchMap.Case.Optional.Some.Extensions
{
    ...
    internal class SwitchMap
    {
        ...
        public void Case_bool_predicate_true()
        {
            ...
            switchCase._defaultOrSelectedMapFunction(switchCase._subject).Should().Be(_TRUE.ToOptional());
        }
        ...
        public void Case_func_predicate_true()
        {
            ...
            switchCase.Case(sbj => sbj == "test", s=> ToTRUE(s));
            switchCase._validPredicatFound.Should().BeTrue();
            switchCase._defaultOrSelectedMapFunction(switchCase._subject).Should().Be(_TRUE.ToOptional());
        }
        ...
        public void Case_multiple_predicate_true()
        {
            ...
            switchCase.Case(true, sbj => ToTRUE(sbj));
            switchCase._validPredicatFound.Should().BeTrue();
            switchCase._defaultOrSelectedMapFunction(switchCase._subject).Should().Be(_TRUE.ToOptional());

            switchCase.Case(() => true, sbj => ToTRUE(sbj, "2"));
            switchCase._validPredicatFound.Should().BeTrue();
            switchCase._defaultOrSelectedMapFunction(switchCase._subject).Should().Be(_TRUE.ToOptional());
