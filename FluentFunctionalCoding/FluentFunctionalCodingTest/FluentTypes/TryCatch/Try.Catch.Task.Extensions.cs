...
namespace FluentFunctionalCodingTest.FluentTypes.Try.Catch.Task.Extensions
{ 
    internal class Try
    {
        Task<Try<string, int, Exception>> GetSuccess(string intValueAsString)
            => new Success<string, int, Exception>(intValueAsString, int.Parse(intValueAsString))
            .As<Try<string, int, Exception>>()
            .ToTask();
        Task<Try<string, int, Exception>> GetFailure(string intValueAsString) 
            => new Failure<string, int, Exception>(intValueAsString, new Exception("generic error"), new Exception("generic exception"))
            .As<Try<string, int, Exception>>()
            .ToTask();
        ...
        {
            GetSuccess("42").Catch((s, e) => e.Message).Result.Do(
        ...
        {
            GetFailure("XX").Catch((s, e) => e.Message).Result.Do(
